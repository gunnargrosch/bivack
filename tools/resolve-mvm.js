...
//   /<stack>/users/<sub>/mvm-identifier
//   /<stack>/users/<sub>/mvm-endpoint
// so the break-glass tools need to know WHICH user's VM to target. Pass the
// user's email via --user <email> (or the BIVACK_USER env var); we look up
// their sub in the Cognito pool, then read the namespaced SSM params.
const { spawnSync } = require('child_process');

const PROFILE = process.env.AWS_PROFILE || 'default';
const REGION = process.env.AWS_REGION || 'us-east-1';
const STACK = process.env.STACK_NAME || 'bivack';
const SSM_PREFIX = `/${STACK}`;
...
// Pull --user <email> from argv (or BIVACK_USER). Returns the email or null.
function userArg(argv) {
  const i = argv.indexOf('--user');
  if (i !== -1 && argv[i + 1]) return argv[i + 1];
  return process.env.BIVACK_USER || null;
}
...
function resolveMvm(email) {
  if (!email) {
    throw new Error(
      ...
      + '  Pass --user <email>, or set BIVACK_USER=<email>.\n'
  ...
  let mvmId, endpoint;
  try {
    mvmId = aws('ssm', 'get-parameter', '--name', `${SSM_PREFIX}/users/${sub}/mvm-identifier`, '--query', 'Parameter.Value');
    endpoint = aws('ssm', 'get-parameter', '--name', `${SSM_PREFIX}/users/${sub}/mvm-endpoint`, '--query', 'Parameter.Value');
  } catch (e) {
