...
- **The VM starts with no AWS credentials.** Its own role (`MicroVmExecutionRole`) only mounts S3 Files and looks up its own endpoint, so it grants nothing against your accounts. To use AWS, authenticate inside the VM: `aws configure sso` once, then `aws sso login --profile <name> --use-device-code`. Name the profile or `--sso-session`, because a bare `aws sso login` resolves the `default` profile. Use `--use-device-code` because the Authorization Code flow redirects to the VM's own `127.0.0.1`, which your browser cannot reach; the device code flow prints a URL and a code you can use on any device. Profiles and credentials persist under `~/.aws` in the home. Provider logins (Claude, Codex, OpenCode, Kiro, and `gh`) also live in each user's home.
